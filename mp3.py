...
def compute_accuracies(predicted_labels, dev_set, dev_labels):
    ...
    tp = np.sum([yhats[i] == dev_labels[i] and yhats[i]
                 == 1 for i in range(len(dev_labels))])
    fp = np.sum([yhats[i] != dev_labels[i] and yhats[i]
                 == 1 for i in range(len(dev_labels))])
    fn = np.sum([yhats[i] != dev_labels[i] and yhats[i]
                 == 0 for i in range(len(dev_labels))])
...
def main(args):
    ...
    # print(dev_labels)
    _, predicted_labels, net = p.fit(
        train_set, train_labels, dev_set, args.max_iter)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='CS440/ECE448 MP3: Neural Nets and PyTorch')
