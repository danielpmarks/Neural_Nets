...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        #self.lin1 = nn.Linear(in_size, 32)
...
def fit(train_set, train_labels, dev_set, n_iter, batch_size=100):
    ...
    for epoch in range(epochs):
        for i in range(n_iter):
            start = 0
            while start + batch_size < len(train_set):
    ...
    #    for j in range(200):
    #
    #    train = train_set[i*batch_size:(i+1)*batch_size]

    outputs = []
    for dev in dev_set:
        outputs.append(net(dev))
    yhats = []
    for out in outputs:
        yhats.append(True if out[1] > out[0] else False)
